...
$isAdminPage = true;
requireLogin(); // kasir dan admin bisa akses

$db = getDB();
$msg = '';
$msgType = 'success';

// Handle actions - kasir hanya bisa tambah & edit produk miliknya (atau semua jika admin)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        $status     = 1; // kasir hanya bisa tambah produk aktif
    ...
    if ($action === 'delete') {
        if (!isAdmin()) {
            $msg = 'Akses ditolak! Hanya admin yang bisa menghapus produk.';
            $msgType = 'danger';
        } else {
            $id = (int)($_POST['id'] ?? 0);
...
<style>
.kasir-layout { display:flex; flex-direction:column; max-width:1200px; margin:0 auto; padding:20px; gap:16px; }
@media(max-width:600px){ .kasir-layout{ padding:12px; } }
</style>

<div class="kasir-layout">
  <div class="page-header">
    <div>
      <div class="page-title"><i class="fa-solid fa-box" style="color:var(--yellow)"></i> Kelola Produk</div>
      <div class="page-subtitle"><?= number_format($totalRows) ?> produk terdaftar</div>
    </div>
    <div class="d-flex gap-2">
      <a href="<?= BASE_URL ?>/pos.php" class="btn btn-secondary"><i class="fa-solid fa-cash-register"></i> Kembali ke Kasir</a>
  ...
  <div class="card" style="padding:14px">
    <form method="GET" class="d-flex gap-2 flex-wrap align-center">
      ...
      <a href="kasir_produk.php" class="btn btn-secondary"><i class="fa-solid fa-xmark"></i></a>
    </form>
  </div>
    ...
    <div class="table-wrap">
      <table>
        ...
        <tr>
          ...
          <td class="text-right fw-bolder text-yellow"><?= formatRupiah((float)$p['price']) ?></td>
          <td class="text-right">
          ...
          <td>
            <div class="d-flex gap-2">
              <button class="btn btn-sm btn-warning" onclick='editProduct(<?= json_encode($p, JSON_HEX_QUOT | JSON_HEX_TAG | JSON_HEX_APOS) ?>)' title="Edit">
              ...
              <form method="POST" style="display:inline" onsubmit="return confirm('Toggle status produk ini?')">
                ...
                <button type="submit" class="btn btn-sm btn-info" title="Toggle Status"><i class="fa-solid fa-toggle-on"></i></button>
              </form>
      ...
      <div class="m-card" style="flex-direction:column;align-items:stretch;gap:8px">
        <div style="display:flex;align-items:center;gap:12px">
          ...
          <div class="m-card-right">
            <div class="m-card-value text-yellow" style="font-size:13px"><?= formatRupiah((float)$p['price']) ?></div>
        ...
        <div class="m-card-actions">
          <button class="btn btn-sm btn-warning" onclick='editProduct(<?= json_encode($p, JSON_HEX_QUOT | JSON_HEX_TAG | JSON_HEX_APOS) ?>)'>
